...
import { AddPersonIcon, CancelIcon, DeleteIcon, MoreIcon, UpdateIcon } from "../assets/icons"
...
const Authors = () => {
  ...
  return (
    ...
    <div className='authors-table p-[25px] rounded-[20px] '>
      <div className="flex items-center gap-[55px] mb-[23px]">
       <TextNormal extraClass={"font-bold text-[18px]"} title={"Authors Table"} />
        <button onClick={() => setAddMenuOpen(true)} className="cursor-pointer text-white "><AddPersonIcon/></button>
      </div>
      <table className="w-full">
        <thead>
          <tr className="border-b-[1px]w-full ">
            <th className="text-start pl-[5px] text-[10px] leading-[150%] text-[#A0AEC0]">AUTHOR</th>
            <th className="text-start text-[10px] leading-[150%] text-[#A0AEC0]">FUNCTION</th>
            <th className="text-start pl-[10px] text-[10px] leading-[150%] text-[#A0AEC0]">STATUS</th>
            <th className="text-start text-[10px] leading-[150%] text-[#A0AEC0]">EMPLOYED</th>
          </tr>
        </thead>
        <tbody>
          {authors.map(item => (
